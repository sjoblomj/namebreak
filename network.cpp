...
string data; // will hold the contents of the URL we read
list<hitdata> hitqueue; // Queue for hits that havn't been sent.
...
void network_handshake(int mode)
{
    ...
    while(!hitqueue.empty())
    {
        ...
        else if(ret == 1)   // Abandon range and empty the hit queue
            hitqueue.clear();
    ...
    int attemptnumber)
{
    ...
    string url, hittobesent;
    if(mode == 0)
        url = urlmode0 + "usr=" + hostname  + "&os="    + os +
    ...
    else if(mode == 1)
    {
        ...
        sleep(1);
        string first = makeStringTransmittable(FIRSTWORD);
        string last  = makeStringTransmittable(LASTWORD);
        url = urlmode1  + "usr=" + hostname + "&fst=" + first +
                "&lst=" + last + "&num=" + hitno;
    }
    else if(mode == 2)
    {
        string first = makeStringTransmittable(FIRSTWORD);
        string last  = makeStringTransmittable(LASTWORD);
        hittobesent  = makeStringTransmittable(hit);
        url = urlmode2 + "usr=" + hostname + "&fst=" + first + "&lst=" +
                last +  "&pos=" + slashpos + "&hit=" + hittobesent +
                "&num=" + hitno;
    }
    else if(mode == 3)
    {
        ...
        string last  = makeStringTransmittable(LASTWORD);
        hittobesent  = makeStringTransmittable(hit);
        url = urlmode3 + "usr=" + hostname + "&fst=" + first + "&lst=" +
                last +  "&pos=" + slashpos + "&match=" + hittobesent +
                "&num=" + hitno;
    }



    // Send the data and retrieve the page on the server.
    CURL* curl;
